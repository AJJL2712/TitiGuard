...
export default function Login() {
    ...
    const handle2FASubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true)
        try {
            const data = await authService.validate2FA(userId, totpCode)
            localStorage.setItem('accessToken', data.accessToken)
            localStorage.setItem('refreshToken', data.refreshToken)
            navigate('/dashboard')
        } catch (err: any) {
            ...
            >
                ...
                >
                    ...
                    <form onSubmit={handle2FASubmit} className="space-y-4">
                        <div>
                            ...
                            <input
                                ...
                                className="w-full text-sm rounded-xl px-4 py-3 outline-none text-center tracking-widest text-lg"
                                style={{
                    ...
                    <button
                        ...
                        className="w-full text-xs text-center mt-4 hover:underline"
                        style={{ color: '#86868b' }}
                    >
                        ← Volver al login
                    </button>
        ...
        >
            ...
            >
                ...
                <p className="text-xs text-center mt-6" style={{ color: '#86868b' }}>
                    ¿No tienes cuenta?{' '}
                    <Link to="/register" style={{ color: '#a896e8' }} className="hover:underline">
                        Crear cuenta gratis
                    </Link>
                </p>
            </div>
